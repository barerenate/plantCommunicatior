...
void measure(int count){
  ...
  // check if plant must be watered
  //ArduinoCloud.update();  //do we need this?
  //moisture = get_average_moisture(); //probably also don't need this
  //if (getMoisture() > threshold) {  
...
void loop() {
  ...
  int a = 20; //number of loops to measure 
  measure(a);
  ...
  double sunnyMin = sunnyValues.size() * 3;
  //List of shade Values
  double shadowMin = shadowValues.size() * 3;
  //List of dark Values
  double darkMin = darkValues.size() * 3;
    ...
    /* const char *INDEX_HTML = R"=====(
      <!DOCTYPE HTML>
      <html>
      <p style = "color:red;">Seconds of sunshine: "sunnyMarker"</p>
      <br>
      <p style = "color:red;">Seconds of shade: "shadeMarker"</p>
      <br>
      <p style = "color:red;">Seconds of darkness: "darknessMarker"</p>
      <br>
      <p style = "color:blue;">Moisture: "moistureMarker"</p>
      <br>
      <p style = "color:yellow;">Average temperature: "meanMarker"</p>
      <br>
  ...
  */
    ...
    delay(10); //does this need to be longer??
